...
async function main() {
  ...
  const tx = api.tx.polkadotXcm.limitedReserveTransferAssets(dest, beneficiaryLocation, assets, 0, "Unlimited");
  ...
  console.log(`Submitting limitedReserveTransferAssets from ${relayer.address}...`);
  console.log(`Nonce: ${nonce}`);
  console.log(`Extrinsic hash: ${signed.hash.toHex()}`);

  await new Promise<void>(async (resolve, reject) => {
    const timeout = setTimeout(() => {
      reject(new Error("Timed out waiting for limitedReserveTransferAssets finalization"));
    }, 120000);
    ...
    const unsub = await signed.send((result) => {
      ...
      if (result.isError) {
        clearTimeout(timeout);
        unsub();
        reject(new Error("limitedReserveTransferAssets submission returned an error status"));
      }
    });
  });
